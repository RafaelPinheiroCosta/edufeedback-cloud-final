...
resource functions 'Microsoft.Web/sites@2024-11-01' = [
  for app in apps: {
    name: app.name
    location: location
    tags: union(tags, {
      ...
      siteConfig: {
        ...
        appSettings: [
          ...
          }
          {
            ...
            value: '@Microsoft.KeyVault(SecretUri=${databaseUserSecretUri})'
          }
          {
          ...
          {
            name: 'EMAIL_SENDER'
            value: '@Microsoft.KeyVault(SecretUri=${emailSenderSecretUri})'
          }
          {
            name: 'ADMIN_EMAIL'
            value: adminEmail
...
output names array = [
  for (app, i) in apps: functions[i].name
]

output principalIds array = [
  for (app, i) in apps: functions[i].identity.principalId
]
