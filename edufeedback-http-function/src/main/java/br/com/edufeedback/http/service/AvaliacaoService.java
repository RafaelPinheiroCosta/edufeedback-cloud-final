...
public class AvaliacaoService {
  ...
  @Inject AvaliacaoPersistenceService persistence;
  @Inject QueuePublisher publisher;
  @Inject MeterRegistry metrics;
  @Inject AvaliacaoMapper mapper;

  public CreationResult criar(CriarAvaliacaoRequest request, UUID correlationId) {
    ...
    LOG.infof(
        "event=feedback.created feedbackId=%s urgency=%s correlationId=%s transaction=committed",
    ...
    if (entity.urgencia == Urgencia.CRITICA) {
      ...
      LOG.infof(
          "event=queue.message.published eventId=%s feedbackId=%s correlationId=%s afterCommit=true",
          event.eventId(), entity.id, correlationId);
      metrics.counter("queue.messages.published").increment();
    }

    return new CreationResult(mapper.toResponse(entity), eventId);
  }
  ...
  public record CreationResult(AvaliacaoResponse response, UUID eventId) {}
}
