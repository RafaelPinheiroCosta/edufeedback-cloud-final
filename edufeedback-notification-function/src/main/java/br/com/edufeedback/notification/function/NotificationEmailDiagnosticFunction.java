...
public class NotificationEmailDiagnosticFunction {
  ...
  @FunctionName("sendNotificationEmailDiagnostic")
  public HttpResponseMessage run(
      @HttpTrigger(
              name = "request",
              methods = {HttpMethod.POST},
              authLevel = AuthorizationLevel.FUNCTION,
              route = "diagnostics/notifications/email")
    ...
    try {
      ...
      String subject =
          input.subject() == null || input.subject().isBlank()
              ? "Diagnóstico direto da Notification Function"
              : input.subject().trim();
      String html =
          input.html() == null || input.html().isBlank()
              ? "<h1>EduFeedback</h1><p>Teste direto do Azure Communication Services Email.</p>"
              : input.html();
      ...
      context
          .getLogger()
          .info(
              "event=diagnostic.notification.email.succeeded operationId="
                  + result.operationId()
                  + " providerStatus="
                  + result.status());
      return json(
      ...
      context
          .getLogger()
          .severe(
              "event=diagnostic.notification.email.failed errorClass="
      ...
      return json(
          ...
          DiagnosticErrorResponse.from("DIAGNOSTIC_EMAIL_FAILED", failure));
    }
  }
